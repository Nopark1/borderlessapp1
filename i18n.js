/* ============================================================
   Borderless — i18n (window.BL_I18N)
   UI strings for the EN / JP toggle. Content strings (event
   copy, member names) live in data.js as {en, jp} pairs.
   ============================================================ */
(function () {
  const S = {
    // shell
    ...
    heroSub:  { en: "An international circle hosting a new gathering every week — temples, ramen, rooftops and riverbanks. Come as you are.",
    ...
    authPerk: { en: "Track your points, RSVP in one tap, and unlock rewards.",
    ...
    pointsRule:{ en: "Attend an event and earn 1 point for every ¥100 of the entry fee.", jp: "イベントに参加すると、参加費¥100ごとに1ポイント獲得。" },
  ...
  const langName = (e) => ({ en: "English", jp: "日本語" });
  const NOW = new Date("2026-06-10T00:00:00");
  window.BL_I18N = {
    t: (key, lang) => (S[key] ? S[key][lang] || S[key].en : key),
    val: (obj, lang) => (obj ? obj[lang] || obj.en : ""),
    daysUntil: (iso) => {
      const d = new Date(iso + "T00:00:00");
      return Math.round((d - NOW) / 86400000);
    },
    relDay: (iso, lang) => {
      const n = Math.round((new Date(iso + "T00:00:00") - NOW) / 86400000);
